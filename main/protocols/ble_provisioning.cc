...
static const struct ble_gatt_chr_def gatt_svr_characteristics[] = {
    {
        ...
        .val_handle = &(instance_ptr->characteristic_val_handle_)
    },
...
void BleProvisioning::start() {
    ...
    // Set the device name
    rc = ble_svc_gap_device_name_set("XiaoZhi-Provisioning");
    if (rc != 0) {
...
void BleProvisioning::advertise() {
    ...
    const uint8_t adv_data_val[] = {
        0x02, 0x01, 0x06, // Flags: LE General Discoverable Mode, BR/EDR Not Supported
        0x03, 0x03, 0xAA, 0xAA // 16-bit service UUID (shortened from 128-bit)
    };
    
    int rc = ble_gap_adv_set_data(adv_data_val, sizeof(adv_data_val));
    if (rc != 0) {
        ESP_LOGE(TAG, "ble_gap_adv_set_data failed: %d", rc);
        return;
    }

    rc = ble_gap_adv_start(own_addr_type_, NULL, BLE_HS_FOREVER, &adv_params, gap_event_cb, this);
    if (rc != 0) {
...
int BleProvisioning::gatt_svr_access_cb(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
    if (instance_ptr && attr_handle == instance_ptr->characteristic_val_handle_) {
...
void BleProvisioning::handle_gatt_write(uint16_t conn_handle, struct os_mbuf *om) {
    // Placeholder for handling incoming JSON data
    uint16_t len = OS_MBUF_PKTLEN(om);
    char* data = (char*)malloc(len + 1);
    if (data) {
        int rc = ble_hs_mbuf_to_flat(om, data, len, NULL);
        if (rc == 0) {
            data[len] = '\0';
            ESP_LOGI(TAG, "Data received: %s", data);
            // Here we will parse the JSON and dispatch commands
        }
        free(data);
    }
}
...
void BleProvisioning::send_data(const std::string& json_data) {
    ...
    int rc = ble_gatts_notify_custom(conn_handle_, characteristic_val_handle_, om);
    if (rc != 0) {
